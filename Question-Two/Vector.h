...
private:
	...
	T *m_data;	// pointer to dynamic array for keeping vector
};
...
inline void Vector<T>::Resize(int newSize)
{
	...
	try
	{
		...
		// if new size is larger, add new elements
		for (size_t i = less; i < newSize; i++)
		{
			m_data[i] = T();
		}
...
{
	if (index < 0 || index > m_size) // check correct index, not out-of-bound
	{
